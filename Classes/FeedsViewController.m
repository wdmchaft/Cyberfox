...
- (id)initWithTitle:(NSString *)title withNavigationTitle:(NSString *)navTitle withPropertyFile:(NSString *)propFile {
	...
	if (self) {
		...
		NSString *localeDatabase = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:propFile];
...
//	[tvCell setMainLabel:@"Updating Feed ..."];
	...
	UIActivityIndicatorView *aiView = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(8, 12, 24, 24)];
//	UILabel *refreshingLabel = [[UILabel alloc] initWithFrame:CGRectMake(40, 12, 200, 24)];
	
//	[refreshingLabel setText:@"Updating Feed ..."];
//	[refreshingLabel setBackgroundColor:[UIColor clearColor]];
//	[refreshingLabel setFont:[UIFont boldSystemFontOfSize:20]];
//	[refreshingLabel setTextColor:[UIColor blackColor]];
	[tvCell setAccessoryType:UITableViewCellAccessoryNone];
	[tvCell setSelectionStyle:UITableViewCellSelectionStyleNone];
//	[tvCell.contentView addSubview:refreshingLabel];
	
	[aiView setActivityIndicatorViewStyle:UIActivityIndicatorViewStyleGray];
	[aiView startAnimating];
	
	[tvCell.contentView addSubview:aiView];
